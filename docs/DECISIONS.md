...
트레이드오프는 결제 중 서버가 종료되면 PENDING 주문과 차감된 DB 재고가 남을 수 있다는 점이다. 현재 구현은 이를 자동 복구하지 않고, 운영에서 미완료 주문 복구 대상으로 명시한다.
...
| 오래된 PENDING 주문 복구 | 현재 요청 흐름 안의 동기 보상에 집중 | 스케줄러/worker, PG 결과 조회, 관리자 알림 |
...
이 위험들은 숨기지 않고 `docs/04-fault-tolerance.md`와 [`06-long-term-roadmap.md`](./06-long-term-roadmap.md)에서 운영 과제로 추적한다.
