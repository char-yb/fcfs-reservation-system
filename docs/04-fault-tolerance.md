...
| API 서버가 결제 중 죽음 | 오래된 PENDING 주문이 남을 수 있음. PG 승인 여부와 포인트 차감 여부를 대조한 뒤 복구해야 함 |

현재 구현은 모든 장애를 자동 복구하지 않는다. 핵심 흐름에서는 오버셀링 방지와 보상 가능한 상태 기록을 우선하고, 서버 종료로 남은 미완료 주문은 장기 개선 항목에서 별도 복구 루프로 다룬다.
...
| `RedisLockClient.getLock` | Redisson fair lock 조회 |
| `RedisLockClient.tryLock` | Redisson fair lock tryLock |
...
Redisson fair lock은 `leaseTime=5s` 후 만료된다. lock 범위가 DB 예약 구간으로 짧기 때문에 결제 전체를 lock으로 붙잡는 구조보다 영향이 작다.
