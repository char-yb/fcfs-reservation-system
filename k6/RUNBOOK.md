...
- 정합성 검증 대상 상품은 `k6/scripts/verify-local-invariants.sh` 상단의 `product_id`로 맞춘다.

### 주요 변수 설명

| 변수 | 의미 | 조정 기준 |
|---|---|---|
| `product_id` / `productId` | 부하테스트 대상 상품 ID | 시드 스크립트, k6 공통 설정, 정합성 검증 스크립트가 모두 같은 값을 써야 한다. |
...
- 정상 완료된 테스트 뒤 Redis `stock:{productId}`가 DB remaining과 같다.
...
| Redis stock drift | Redis `stock:{productId}`와 DB remaining 비교, L1 카운터 복구 로그 |
